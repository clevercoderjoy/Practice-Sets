...
// create an array of objects where the key is the id of the video and the value are an array of comment text to that video
const res = (videoJSON) => videoJSON.videos.map((item) => ({[item.id] : item.comments.map((users) => users.comment)}));
console.log(res(videoJSON));